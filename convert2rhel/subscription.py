...
_CENTOS_6_REPO_CONTENT = \
        ...
        'mirrorlist=https://vault.centos.org/centos/6/contrib/$basearch/\n' \
        'gpgcheck=0\n' \
        'enabled=1\n'
...
_UBI_8_REPO_CONTENT = \
        ...
        'mirrorlist=https://cdn-ubi.redhat.com/content/public/ubi/dist/ubi8/8/$basearch/baseos/os/\n' \
        'gpgcheck=0\n' \
        'enabled=1\n'
...
def download_rhsm_pkgs():
    ...
    The packages are available on CentOS only, so additional repofiles need to be used on non-CentOS systems.
    The downloaded RPMs are to be installed in a later stage of the conversion.
    """
...
def _download_rhsm_pkgs(pkgs_to_download, repo_path, repo_content):
    ...
    if system_info.id != "centos" or system_info.version.major == 6:
        # On systems other than CentOS 7 or 8, we need to create a CentOS repofile that holds the RHSM pkgs.
        # On RHEL 6-based systems, we need to create a repofile for CentOS 6 Contrib repo that holds the RHSM pkgs. This
        #  repo is not available/enabled on most systems.
        ...
        paths = utils.download_pkgs(pkgs_to_download, dest=downloaddir)
...
def _get_rhsm_cert_on_centos_7():
    ...
    output, ret_code = utils.run_subprocess("cpio -F %s -iv --to-stdout .%s" % (cpio_filepath, cert_path),
                                            print_output=False)
    if ret_code != 0:
