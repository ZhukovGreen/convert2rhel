...
REPOQUERY_F_STUB_BAD = (
    b"kernel-idontexpectyou-core-sdsdsd.ell8_3.x86_64\n"
    b"kernel-core-0:4.18.0-240.15.1.el8_3.x86_64\n"
    b"kernel-debug-core-0:4.18.0-240.10.1.el8_3.x86_64\n"
    b"kernel-debug-core-0:4.18.0-240.15.1.el8_3.x86_64\n"
)
...
def test_ensure_compatibility_of_kmods(
    ...
    if host_kmods == HOST_MODULES_STUB_BAD:
        with pytest.raises(SystemExit):
            ensure_compatibility_of_kmods(_refresh_system_info=True)
    else:
        ensure_compatibility_of_kmods(_refresh_system_info=True)
...
def test_get_rhel_supported_kmods(
    ...
    if exception:
        with pytest.raises(exception):
            get_rhel_supported_kmods(_refresh_system_info=True)
    else:
        res = get_rhel_supported_kmods(_refresh_system_info=True)
        assert res == set(
            (
                "kernel/lib/a.ko.xz",
                "kernel/lib/b.ko.xz",
